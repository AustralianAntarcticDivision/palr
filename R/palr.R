...
## two-character lowercase hex for an opacity value in [0, 1]
.alpha_hex <- function(alpha) {
  sprintf("%02x", pmax(0L, pmin(255L, as.integer(round(255 * alpha)))))
}

## Shared worker for the absolute palettes (sst, chl, ice, bathy, dirty).
##
## 'breaks' and 'cols' are parallel vectors of data values and colours.
## Implements the three modes documented on each palette function:
##   pal(n)              n colours from the ramp
##   pal(data)           colours matched to data values (length preserved,
##                       values below the first break clamp to the first
##                       colour, values above the last break clamp to the
##                       last colour, missing values return NA)
##   pal(palette = TRUE) list of breaks and cols
##   pal()               a colorRampPalette function
abs_pal <- function(x, palette = FALSE, alpha = 1, breaks, cols) {
  cols <- paste0(cols, .alpha_hex(alpha))
  if (palette) return(list(breaks = breaks, cols = cols))
  if (missing(x)) return(colorRampPalette(cols, alpha = TRUE))
  if (length(x) == 1L) {
    return(colorRampPalette(cols, alpha = TRUE)(x))
  }
  idx <- pmax(1L, findInterval(x, breaks))
  cols[idx]
}
