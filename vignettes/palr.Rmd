...
The default plot colours uses a setting provided by the raster package, but we have SST data in degrees Celsius so we can use the
`sstPal` function to give specific colours for particular temperatures. The full range of the temperatures is shown on the plot legend, even though our data only has values in the range `r format(cellStats(oisst, range), digits = 2)`. 
...
Full ncdump metadata from the data sets can be obtained with 
